...
class ObsManager(ObsManagerBase):
  ...
  def attach_ego_vehicle(self, vehicle, criteria_stop):
    ...
    if self._town == "Town11":
      ...
      self._lane_marking_all = cv.resize(self._lane_marking_all, (0, 0), fx=0.5, fy=0.5, interpolation=cv.INTER_NEAREST)
      self._lane_marking_white_broken = cv.resize(self._lane_marking_white_broken, (0, 0), fx=0.5, fy=0.5, interpolation=cv.INTER_NEAREST)
      self._sidewalk = cv.resize(self._sidewalk, (0, 0), fx=0.5, fy=0.5, interpolation=cv.INTER_NEAREST)
      self._pixels_per_meter /= 2
      self._width //= 2
      self._pixels_ev_to_bottom /= 2
  ...
  def get_observation(self, close_traffic_lights=None):
    ...
    if self._town == "Town11":
      c_all = cv.resize(c_all, (0, 0), fx=2, fy=2, interpolation=cv.INTER_NEAREST)
